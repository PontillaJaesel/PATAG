...
// --- GLOBAL STATIC CONFIG DATA ---
const industries = [
  "Education", "Healthcare", "Government", "IT / Technology",
  "Business / Finance", "Agriculture", "Transportation",
  "Manufacturing", "Unemployed", "Retired", "Student", "Others"
];

const voterTypes = [
  "Regular Voter", "SK Voter", "First-time Voter",
  "Overseas Voter", "Senior Citizen", "PWD", "Not a Voter"
];

const locationData: Record<string, string[]> = {
  "Batangas": ["Batangas City", "Lipa City", "Tanauan City", "Santo Tomas", "Bauan", "San Pascual", "Lemery", "Nasugbu"],
  "Cebu": ["Cebu City", "Lapu-Lapu City", "Mandaue City", "Talisay City", "Toledo City", "Bogo City", "Carcar City"],
  "Metro Manila": ["Manila", "Quezon City", "Makati City", "Taguig City", "Pasig City", "Mandaluyong City", "Paranaque City", "Caloocan City", "Valenzuela City", "Muntinlupa City"],
  "Cavite": ["Bacoor City", "Dasmarinas City", "Imus City", "Tagaytay City", "Trece Martires City", "General Trias City", "Carmona", "Silang", "Kawit"],
  "Laguna": ["Calamba City", "Santa Rosa City", "Binan City", "San Pedro City", "Cabuyao City", "San Pablo City", "Los Banos", "Santa Cruz"],
  "Rizal": ["Antipolo City", "Cainta", "Taytay", "San Mateo", "Rodriguez", "Binangonan", "Angono"],
  "Bulacan": ["Malolos City", "Meycauayan City", "San Jose del Monte City", "Baliuag City", "Marilao", "Bocaue", "Santa Maria"],
  "Pampanga": ["San Fernando City", "Angeles City", "Mabalacat City", "Guagua", "Lubao", "Porac", "Apalit"],
  "Pangasinan": ["Dagupan City", "Urdaneta City", "Alaminos City", "San Carlos City", "Lingayen", "Calasiao", "Mangaldan"],
  "Iloilo": ["Iloilo City", "Passi City", "Oton", "Pototan", "Pavia", "Santa Barbara", "Miagao"],
  "Negros Occidental": ["Bacolod City", "Bago City", "Cadiz City", "Kabankalan City", "San Carlos City", "Silay City", "Talisay City"],
  "Davao del Sur": ["Davao City", "Digos City", "Bansalan", "Hagonoy", "Malalag", "Matanao", "Santa Cruz"],
  "Misamis Oriental": ["Cagayan de Oro City", "Gingoog City", "El Salvador City", "Tagoloan", "Villanueva", "Opol", "Balingasag"]
};
        ...
        <Card>
          ...
          <form
            ...
            onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
              ...
              const referenceDate = new Date(2026, 4, 21);
              ...
              if (calculatedAge < 14) setRole("researcher");
              setStep(2);
            }}
          >
            ...
            <Field
              icon={
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="hover:text-cream transition-colors focus:outline-none">
            ...
            <Field
              icon={
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="hover:text-cream transition-colors focus:outline-none">
            ...
            {error && <div className="rounded-lg bg-red-500/10 p-3 text-center text-xs font-medium text-red-400 border border-red-500/20">{error}</div>}
        ...
        <Card className="max-w-xl">
          ...
          {role === "citizen" && (
            <CitizenForm
              ...
              specificWork={specificWork} setSpecificWork={setSpecificWork}
              province={province} setProvince={setProvince}
              city={city} setCity={setCity}
              voterType={voterType} setVoterType={setVoterType}
          ...
          {role === "researcher" && (
            <ResearcherForm city={city} setCity={setCity} idFile={idFile} setIdFile={setIdFile} onSubmit={finish} />
          )}

          {role === "journalist" && (
            <JournalistForm city={city} setCity={setCity} idFile={idFile} setIdFile={setIdFile} onSubmit={finish} />
          )}
...
// --- SUB-FORMS: MOVED OUTSIDE MAIN COMPONENT TO ELIMINATE RE-CREATION REMOUNT FOCUS DROP BUGS ---

interface CitizenFormProps {
  selectedIndustry: string; setSelectedIndustry: (v: string) => void;
  specificWork: string; setSpecificWork: (v: string) => void;
  province: string; setProvince: (v: string) => void;
  city: string; setCity: (v: string) => void;
  voterType: string; setVoterType: (v: string) => void;
...
function CitizenForm({
  ...
  specificWork, setSpecificWork,
  province, setProvince,
  city, setCity,
  voterType, setVoterType,
  idFile, setIdFile,
  onSubmit
}: CitizenFormProps) {
  return (
    <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
      ...
      <div className="mt-5 space-y-4">
        <Labeled label="Occupational Industry">
          <select className="auth-select" value={selectedIndustry} onChange={(e) => setSelectedIndustry(e.target.value)} required>
            <option value="" disabled>Select industry</option>
            {industries.map((ind) => <option key={ind} value={ind}>{ind}</option>)}
          </select>
        </Labeled>

        <div className={selectedIndustry ? "block animate-in fade-in duration-200" : "hidden"}>
          <Labeled label={selectedIndustry === "Others" ? "Specify Industry" : "Specific Work / Position"}>
            <input
              type="text"
              required={selectedIndustry !== ""}
              value={specificWork}
              onChange={(e) => setSpecificWork(e.target.value)}
              placeholder={selectedIndustry === "Others" ? "Please specify your industry/work" : "e.g. Teacher, Nurse, Software Developer"}
              className="auth-input border-b border-white/20 pb-2 w-full text-cream bg-transparent outline-none placeholder:text-cream/30"
            />
          </Labeled>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Labeled label="Province">
            <select className="auth-select" value={province} onChange={(e) => { setProvince(e.target.value); setCity(""); }} required>
          ...
          <Labeled label="City / Municipality">
            <div className="flex items-center gap-2 border-b border-white/20 pb-2">
              ...
              <select className="w-full bg-transparent text-cream focus:outline-none appearance-none" value={city} onChange={(e) => setCity(e.target.value)} required disabled={!province}>
                <option value="" disabled className="bg-onyx text-cream/40">{province ? "Select city" : "Select province first"}</option>
                {province && locationData[province].map(c => <option key={c} value={c} className="bg-onyx text-cream">{c}</option>)}
              </select>
            </div>
          </Labeled>
        </div>
        ...
        <Labeled label="Voter Type">
          <select className="auth-select" value={voterType} onChange={(e) => setVoterType(e.target.value)} required>
            <option value="" disabled>Select voter type</option>
            {voterTypes.map((type) => <option key={type} value={type}>{type}</option>)}
          </select>
        </Labeled>
...
function ResearcherForm({ city, setCity, idFile, setIdFile, onSubmit }: { city: string; setCity: (v: string) => void; idFile: File | null; setIdFile: (f: File | null) => void; onSubmit: (x: Record<string, any>) => void }) {
  return (
    <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); onSubmit({ city, idFile }); }} className="mt-4">
      ...
      <div className="mt-5 space-y-4">
        ...
        <Labeled label="Institution or Organization Name">
          <input required value={city} onChange={(e) => setCity(e.target.value)} className="auth-input border-b border-white/20 pb-2 w-full" placeholder="e.g. BatStateU Alangilan" />
        </Labeled>
        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
...
function JournalistForm({ city, setCity, idFile, setIdFile, onSubmit }: { city: string; setCity: (v: string) => void; idFile: File | null; setIdFile: (f: File | null) => void; onSubmit: (x: Record<string, any>) => void }) {
  return (
    <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); onSubmit({ city, idFile }); }} className="mt-4">
      ...
      <div className="mt-5 space-y-4">
        <Labeled label="Institution / Publication Name">
          <input required value={city} onChange={(e) => setCity(e.target.value)} className="auth-input border-b border-white/20 pb-2 w-full" placeholder="e.g. Philippine Daily Inquirer" />
        </Labeled>
